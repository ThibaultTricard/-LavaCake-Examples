...
int main() {
	...
	for (uint32_t u = 0; u < UINT32_MAX; u++) {
	...
	d->initDevices(0, 1, w.m_windowParams);
	{
		...
		vec4f camera = vec4f({ 0.f,0.f,4.f,0.0f });
		constant->addVariable("camera", camera);

		constantDescription constantInfo;
		constantInfo.constantShader = VK_SHADER_STAGE_FRAGMENT_BIT;
		constantInfo.constantSize = constant->size();

		teapotPipeline->setPushContantInfo({ constantInfo });
		teapotPipeline->setVertices({ {teapot_vertex_buffer, {{constant, VK_SHADER_STAGE_FRAGMENT_BIT}}} });
		...
		delete frag;
		delete vertex;
		delete teapotPipeline;
		delete skyboxFrag;
		delete skyboxVertex;
		delete skybox;
		delete b;
		delete teapot_vertex_buffer;
		delete teapot_mesh;
		delete v;
		delete sky_mesh;
		delete cubeMap;
